...
describe('CSDL Tests', () => {
  ...
  files.forEach((file) => {
    describe(file, () => {
      ...
      it('Properties have proper Nullable usage', () => {propertyNullCheck(csdl);});
...
function propertyNullCheck(csdl) {
    // Check Navigation Properties for proper usage
    let navProps = CSDL.search(csdl, 'NavigationProperty');
    for(let i = 0; i < navProps.length; i++) {
      let navProp = navProps[i];

      // All Navigation Properties of "Collection( X )" cannot specify Nullable
      if(navProp.Type.startsWith('Collection(') && navProp.Nullable !== undefined) {
        throw new Error('NavigationProperty "'+navProp.Name+'" cannot specify Nullable!');
      }

      // All other Navigation Properties require Nullable="false"
      if(!navProp.Type.startsWith('Collection(') && navProp.Nullable !== false) {
        throw new Error('NavigationProperty "'+navProp.Name+'" requires Nullable="false"!');
      }
    }

    // Check Properties for proper usage
    let props = CSDL.search(csdl, 'Property');
    for(let i = 0; i < props.length; i++) {
      let prop = props[i];
      let isCollection = false;

      // Get the underlying type for the property
      ...
      // All Properties that are using ComplexType require Nullable="false" if not in a array
      // If in an array, Nullable may be required if the array is read-only
      if(type.constructor.name === 'ComplexType' && prop.Nullable !== false && isCollection === false) {
        throw new Error('Property "'+prop.Name+'" requires Nullable="false"!');
      }
    }
}
