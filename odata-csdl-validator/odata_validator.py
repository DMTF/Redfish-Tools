...
class Element(object):
    ...
    def __init__(self, data, parent):
        ...
        try:
            self.parse()
            self._check_for_extra_elements()
    ...
    def _get_required_attrib(self, attrib_name):
        ...
        if attrib_name not in self.raw_data.attrib:
            ...
            self.used_attribs.append(attrib_name)
    ...
    def _check_for_extra_elements(self):
        """Checks for unparsed XML elements.
        ...
        """
        if not isinstance(self.raw_data, ET.Element):
            return

        parsed_xml = [child.raw_data for child in self.children]
        unparsed_xml = [x.tag for x in self.raw_data if x not in parsed_xml]

        if unparsed_xml:
            raise SchemaError("Unparsed child XML {}".format(unparsed_xml))

        unparsed_attribs = [x for x in self.raw_data.attrib if x not in self.used_attribs]

        if unparsed_attribs:
            raise SchemaError("Unused attributes {}".format(unparsed_attribs))
...
class MetaData(Element):
    ...
    def __init__(self, uri):
        ...
        try:
            ...
            if ( "http://docs.oasis-open.org/odata/odata/v4.0/os/vocabularies/Org.OData.Core.V1.xml" not in error.message and
                "http://docs.oasis-open.org/odata/odata/v4.0/cs01/vocabularies/Org.OData.Core.V1.xml" not in error.message ):
                print(error.message)
                sys.exit(0)
        except Exception:
            raise
...
class Include(Element):
    ...
    def parse(self):
        ...
        if 'Alias' in self.raw_data.attrib:
            self.used_attribs.append('Alias')
...
class Schema(Element):
    ...
    def parse(self):
        ...
        self.used_attribs.append('Namespace')
...
class Property(Element):
    ...
    def parse(self):
        ...
        if not isinstance(self.parent, StructuredType):
            raise SchemaError("Property not a child of a structured type")
        ...
        if 'MaxLength' in self.raw_data.attrib:
            self.used_attribs.append('MaxLength')
        ...
        if 'Precision' in self.raw_data.attrib:
            self.used_attribs.append('Precision')
            try:
        ...
        if 'Scale' in self.raw_data.attrib:
            self.used_attribs.append('Scale')
        ...
        if 'Unicode' in self.raw_data.attrib:
            self.used_attribs.append('Unicode')
        ...
        if 'DefaultValue' in self.raw_data.attrib:
            self.used_attribs.append('DefaultValue')
    ...
    def check_scope(self):
        ...
        if isinstance(type_element, AbstractType):
            ...
            if not (isinstance(type_element, PrimitiveType) or
                    isinstance(type_element, ComplexType) or isinstance(type_element, EnumType)):
                raise SchemaError("Type must be PrimitiveType, ComplexType or EnumType in scope")
...
class NavigationProperty(Element):
    ...
    def parse(self):
        ...
        if 'Partner' in self.raw_data.attrib:
            self.used_attribs.append('Partner')
        ...
        if 'ContainsTarget' in self.raw_data.attrib:
            self.used_attribs.append('ContainsTarget')
...
class EntityType(Element, StructuredType):
    ...
    def parse(self):
        ...
        if 'OpenType' in self.raw_data.attrib:
            self.used_attribs.append('OpenType')
        ...
        if 'HasStream' in self.raw_data.attrib:
            self.used_attribs.append('HasStream')
...
class ComplexType(Element, StructuredType):
    ...
    def check_scope(self):
        ...
        if self.base_type:
            ...
            base_type_chain = [base_type]

            current_element = base_type
            while current_element.base_type:
                new_element = current_element.find_in_scope(current_element.base_type)
                if new_element in base_type_chain:
                    raise SchemaError("Inheritance cycle introduced with base type {}->{}".format(
                        current_element.name, new_element.name))
                base_type_chain.append(new_element)
                current_element = new_element

            for base_type in base_type_chain:
            ...
            self.nav_properties = copy.copy(self.defined_nav_properties)
            prop_names = [x.name for x in self.properties] + [x.name for x in self.nav_properties]
            for element in base_type_chain:
                if element != self:
                    for prop in element.defined_properties:
                        if prop.name in prop_names:
                            raise SchemaError("BaseType {} property name {} not unique".format(
...
class EnumType(Element, PrimitiveType):
    ...
    def parse(self):
        ...
        if 'UnderlyingType' in self.raw_data.attrib:
            self.used_attribs.append('UnderlyingType')
        ...
        if 'IsFlags' in self.raw_data.attrib:
            self.used_attribs.append('IsFlags')
...
class Member(Element):
    ...
    def parse(self):
        ...
        self.used_attribs.append('Value')
...
class Action(Element):
    ...
    def parse(self):
        ...
        if 'IsBound' in self.raw_data.attrib:
            self.used_attribs.append('IsBound')
        ...
        if 'EntitySetPath' in self.raw_data.attrib:
            self.used_attribs.append('EntitySetPath')
...
class Function(Element):
    ...
    def parse(self):
        ...
        if 'IsComposable' in self.raw_data.attrib:
            self.used_attribs.append('IsComposable')
...
class EntityContainer(Element):
    ...
    def parse(self):
        ...
        if 'Extends' in self.raw_data.attrib:
            self.used_attribs.append('Extends')
...
class EntitySet(Element):
    ...
    def parse(self):
        ...
        if 'IncludeInServiceDocument' in self.raw_data.attrib:
            self.used_attribs.append('IncludeInServiceDocument')
...
class ActionImport(Element):
    ...
    def parse(self):
        ...
        if 'EntitySet' in self.raw_data.attrib:
            self.used_attribs.append('EntitySet')
...
class Term(Element):
    ...
    def parse(self):
        ...
        if 'BaseTerm' in self.raw_data.attrib:
            self.used_attribs.append('BaseTerm')
        ...
        if 'AppliesTo' in self.raw_data.attrib:
            self.used_attribs.append('AppliesTo')
